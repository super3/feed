...
class Storage {
  constructor(options = {}) {
    ...
    if (appConfig.environment.isVercel && !isUpstashUrl) {
      this.type = options.type || 'local';
    } else {
      this.type = options.type || (hasRedisUrl ? 'redis' : 'local');
    }
    ...
    if (this.type === 'redis' && !this.redis) {
      try {
        if (hasKvCredentials) {
        ...
        } else if (isUpstashUrl && appConfig.storage.effectiveRedisUrl) {
          ...
          this.redis = createClient({ url: appConfig.storage.effectiveRedisUrl });
  ...
  async init() {
    if (this.type === 'local') {
      ...
      if (appConfig.environment.isVercel) {
        this.dataDir = appConfig.paths.dataDir;
      }
      try {

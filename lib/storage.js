...
class Storage {
  constructor(config = {}) {
    ...
    // Check if REDIS_URL is an Upstash URL (contains .upstash.io)
    const isUpstashUrl = (hasRedisUrl && (process.env.REDIS_URL?.includes('.upstash.io') || process.env.KV_URL?.includes('.upstash.io'))) || hasKvCredentials;
    
    // On Vercel, only use Redis if we have Upstash URL
    // Standard Redis connections don't work on serverless
    if (process.env.VERCEL && !isUpstashUrl) {
      this.type = config.type || 'local';
    } else {
      this.type = config.type || (hasRedisUrl ? 'redis' : 'local');
    }
    ...
    if (this.type === 'redis' && !this.redis) {
      try {
        if (hasKvCredentials) {
        ...
        } else if (isUpstashUrl && (process.env.REDIS_URL || process.env.KV_URL)) {
          ...
          // Use standard Redis client for non-Upstash URLs
  ...
  async init() {
    if (this.type === 'local') {
      ...
      if (process.env.VERCEL) {
        this.dataDir = '/tmp/feed-data';
      }
      try {
        await fs.mkdir(this.dataDir, { recursive: true });
      } catch (error) {
        console.error('Error creating data directory:', error);
        // Continue anyway - read operations might still work
      }
    }
  }

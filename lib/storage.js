const fs = require('fs').promises;
const path = require('path');

let kv;
try {
  const kvModule = require('@vercel/kv');
  kv = kvModule.kv;
} catch (e) {
  // @vercel/kv not available in local environment
}

class Storage {
  constructor(config = {}) {
    // Auto-detect storage type based on environment
    this.type = config.type || (process.env.KV_REST_API_URL ? 'kv' : 'local');
    this.dataDir = config.dataDir || path.join(__dirname, '..', 'data');
    
    // Initialize KV client if using Vercel KV
    if (this.type === 'kv' && kv) {
      this.kv = kv;
    }
  }

  async init() {
    if (this.type === 'local') {
      try {
        await fs.mkdir(this.dataDir, { recursive: true });
      } catch (error) {
        console.error('Error creating data directory:', error);
      }
    }
  }

  async get(key) {
    if (this.type === 'local') {
      try {
        const filePath = path.join(this.dataDir, `${key}.json`);
        const data = await fs.readFile(filePath, 'utf8');
        return JSON.parse(data);
      } catch (error) {
        if (error.code === 'ENOENT') {
          return null;
        }
        throw error;
      }
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.get(key);
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }

  async set(key, value, options = {}) {
    if (this.type === 'local') {
      const filePath = path.join(this.dataDir, `${key}.json`);
      await fs.writeFile(filePath, JSON.stringify(value, null, 2));
      return 'OK';
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.set(key, value, options);
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }

  async delete(key) {
    if (this.type === 'local') {
      try {
        const filePath = path.join(this.dataDir, `${key}.json`);
        await fs.unlink(filePath);
        return 1;
      } catch (error) {
        if (error.code === 'ENOENT') {
          return 0;
        }
        throw error;
      }
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.del(key);
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }

  async sadd(key, member) {
    if (this.type === 'local') {
      const set = (await this.get(key)) || [];
      if (!set.includes(member)) {
        set.push(member);
        await this.set(key, set);
        return 1;
      }
      return 0;
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.sadd(key, member);
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }

  async smembers(key) {
    if (this.type === 'local') {
      return (await this.get(key)) || [];
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.smembers(key);
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }

  async sismember(key, member) {
    if (this.type === 'local') {
      const set = (await this.get(key)) || [];
      return set.includes(member) ? 1 : 0;
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.sismember(key, member);
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }

  async keys(pattern) {
    if (this.type === 'local') {
      const files = await fs.readdir(this.dataDir);
      const jsonFiles = files.filter(f => f.endsWith('.json'));
      const keys = jsonFiles.map(f => f.replace('.json', ''));
      
      if (pattern) {
        const regex = new RegExp(pattern.replace(/\*/g, '.*'));
        return keys.filter(key => regex.test(key));
      }
      
      return keys;
    }
    
    if (this.type === 'kv' && this.kv) {
      return await this.kv.keys(pattern || '*');
    }
    
    throw new Error(`Storage type ${this.type} not implemented`);
  }
}

// Singleton instance
let storageInstance;

function getStorage(config) {
  if (!storageInstance) {
    storageInstance = new Storage(config);
  }
  return storageInstance;
}

// Reset function for testing
function resetStorage() {
  storageInstance = null;
}

module.exports = { getStorage, resetStorage };
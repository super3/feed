...
const environmentStrategies = {
  vercel: {
    dataDir: () => process.env.VERCEL_DATA_DIR || '/tmp/feed-data',
    logLevel: () => process.env.LOG_LEVEL || 'info',
    maxWorkers: () => 1 // Vercel functions are single-threaded
  },
  development: {
  ...
  test: {
    dataDir: () => require('path').join(__dirname, '..', 'test-data'),
    logLevel: () => 'silent',
    maxWorkers: () => 1
  }
};

// Get current environment strategy
const getEnvironmentStrategy = () => {
  if (environment.isVercel) return environmentStrategies.vercel;
  if (environment.isTest) return environmentStrategies.test;
  if (environment.isProduction) return environmentStrategies.production;
  return environmentStrategies.development;
};

// Storage paths configuration
const paths = {
  // Data directory for local storage - now uses environment strategy
  get dataDir() {
    return getEnvironmentStrategy().dataDir();
  }
};
...
module.exports = {
  ...
  get logLevel() {
    return getEnvironmentStrategy().logLevel();
  },
  get maxWorkers() {
    return getEnvironmentStrategy().maxWorkers();
  },

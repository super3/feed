...
describe('Storage - KV Implementation', () => {
  let storage;
  let mockKv;
  ...
  beforeEach(() => {
    ...
    // Set KV environment variable to trigger KV mode
  ...
  describe('basic operations', () => {
    it('should use KV for get operation', async () => {
    ...
    it('should use KV for set operation', async () => {
      mockKv.set.mockResolvedValue('OK');
      
      const result = await storage.set('test-key', { test: 'data' });
      
      expect(mockKv.set).toHaveBeenCalledWith('test-key', { test: 'data' }, {});
      expect(result).toBe('OK');
    });

    it('should use KV for delete operation', async () => {
  ...
  describe('set operations', () => {
    it('should use KV for sadd operation', async () => {
      mockKv.sadd.mockResolvedValue(1);
      
      const result = await storage.sadd('myset', 'item1');
      
      expect(mockKv.sadd).toHaveBeenCalledWith('myset', 'item1');
      expect(result).toBe(1);
    });

    it('should use KV for smembers operation', async () => {
      mockKv.smembers.mockResolvedValue(['item1', 'item2']);
      
      const result = await storage.smembers('myset');
      
      expect(mockKv.smembers).toHaveBeenCalledWith('myset');
      expect(result).toEqual(['item1', 'item2']);
    });

    it('should use KV for sismember operation', async () => {
      mockKv.sismember.mockResolvedValue(1);
      
      const result = await storage.sismember('myset', 'item1');
      
      expect(mockKv.sismember).toHaveBeenCalledWith('myset', 'item1');
      expect(result).toBe(1);
    });
  });

  describe('keys operation', () => {
    it('should use KV for keys operation with pattern', async () => {
      mockKv.keys.mockResolvedValue(['posts:js:1234', 'posts:py:5678']);
      
      const result = await storage.keys('posts:*');
      
      expect(mockKv.keys).toHaveBeenCalledWith('posts:*');
      expect(result).toEqual(['posts:js:1234', 'posts:py:5678']);
    });

    it('should use KV for keys operation without pattern', async () => {

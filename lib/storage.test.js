...
describe('Storage - Vercel environment', () => {
  ...
  it('should handle standard Redis client with connection error', async () => {
    ...
    // Give time for async connection to fail
    ...
    // Cleanup
    delete process.env.REDIS_URL;
    process.env.VERCEL = '1'; // Restore VERCEL env
    jest.dontMock('redis');
    jest.resetModules();
  });
  ...
  it('should handle standard Redis operations', async () => {
    ...
    // Remove VERCEL env to test standard Redis
    delete process.env.VERCEL;
    process.env.REDIS_URL = 'redis://regular-redis.com:6379';
    ...
    // Mock @upstash/redis to be a valid module (but won't be used for standard Redis)
    jest.doMock('@upstash/redis', () => ({
      Redis: jest.fn()
    }));

    // Mock redis module for standard Redis
    jest.doMock('redis', () => ({
      createClient: jest.fn(() => mockRedisClient)
    }));
    ...
    // Test sadd (standard Redis uses sAdd with capital A)
    mockRedisClient.sAdd.mockResolvedValue(1);
    const saddResult = await storage.sadd('set', 'member');
    expect(saddResult).toBe(1);

    // Test smembers (standard Redis uses sMembers with capital M)
    ...
    // Test sismember (standard Redis uses sIsMember)
    mockRedisClient.sIsMember.mockResolvedValue(true);
    const isMember = await storage.sismember('set', 'member');
    expect(isMember).toBe(1);

    // Test del (note: storage uses 'delete' method, not 'del')
    mockRedisClient.del.mockResolvedValue(1);
    const delResult = await storage.delete('key');
  ...
  it('should handle errors in standard Redis operations', async () => {
    ...
    const mockRedisClient = {
      ...
      sAdd: jest.fn().mockRejectedValue(new Error('Sadd failed')),
      sMembers: jest.fn().mockRejectedValue(new Error('Smembers failed')),
      sIsMember: jest.fn().mockRejectedValue(new Error('Sismember failed'))
    };
    ...
    await expect(storage.sismember('set', 'member')).rejects.toThrow('Sismember failed');
    await expect(storage.delete('key')).rejects.toThrow('Del failed');
